...
if ((importingError) and not(importingTkError)):
    ...
    lbl = Label(window, text="The following modules are not installed:\n"+importErrStr, justify="left")
...
def ui(im, nim, im_name):
    ...
    imageTab = Frame(note)
    settingsTab1 = Frame(note)
    settingsTab2 = Frame(note)
    savingTab = Frame(note)
    ...
    i = Label(imageTab, image=ui_image, compound="top",
    ...
    f = Frame(settingsTab1)
    #f.configure(background='Red')
    
    f2 = Frame(settingsTab2)
    #g.configure(background='Blue')

    save = Frame(savingTab)
    ...
    def floatentry(f, v):
        var = DoubleVar(f)
        var.set(v)
        w = Entry(f, textvariable=var, validatecommand=validate_float, validate="key", width=10)
        return w, var, w

    def intentry(f, v):
        var = IntVar(f)
        var.set(v)
        w = Entry(f, textvariable=var, validatecommand=validate_int, validate="key", width=10)
        return w, var, w

    def checkbutton(k, v):
        var = BooleanVar(frame)
        var.set(v)
        g = Frame(frame)
        w = Checkbutton(g, variable=var, text=_("Yes"))
        w.pack(side="left")
        return g, var, w 
    ...
    def intscale(k, v, min=1, max = 100):
        var = IntVar(frame)
        var.set(v)
        g = Frame(frame, borderwidth=0)
        w = Scale(g, orient="h", variable=var, from_=min, to=max, showvalue=False)
        l = Label(g, textvariable=var, width=3)
        l.pack(side="left")
    ...
    def intscale2(k, v, min=1):
        ...
        w, h = im.size
        mmax = max(w,h)
        w = Scale(g, orient="h", variable=var, from_=min, to=mmax, showvalue=False)
        l = Label(g, textvariable=var, width=3)
        l.pack(side="left")
        w.pack(side="left", fill="x", expand=1)
        return g, var, w
    ...
    def _optionmenu(k, v, *options):
        ...
        var = IntVar(frame)    
        var.set(v)
        svar = StringVar(frame)
        svar.set(options[v])
        svar.trace("w", trace)
        g = Frame(frame)
        w = ttk.Combobox(g, values=options, textvariable=svar)
        w.current(v)
        w.pack(side="left")	
        return g, var, w
    ...
    def load_settings():
        ...
        if not os.path.isdir(settingsDir):
            path = filedialog.askopenfilename(defaultextension=".image2gcoderc",
                filetypes = (
                    (_("Settings"), ".image2gcoderc"),
                    (_("All files"), "*")))
        else:
            path = filedialog.askopenfilename(initialdir = settingsDir, defaultextension=".image2gcoderc",
                filetypes = (
                    (_("Settings"), ".image2gcoderc"),
                    (_("All files"), "*")))
        if path == "":
        ...
        try:
            ...
            for k, v in defaults.items():
                vars[k].set(v)
                if isinstance(chw[k], ttk.Combobox):
    ...
    status = IntVar()
    bb = Button(b, text=_("START"), command=lambda:status.set(1), width=8, default="active")
    bb.pack(side="left", padx=4, pady=4)
    bb = Button(b, text=_("Cancel"), command=lambda:status.set(-1), width=8, default="normal")
    bb.pack(side="left", padx=4, pady=4)

    lb1 = Label(save,  text="Save current settings as:").grid(row=1, column=1, sticky="ew")
    saveField = Entry(save, textvariable=fileName, width=15)
    saveField.grid(row=2, column=1, sticky="ew")
    saveBtn = Button(save, text=_("Save"), command=save_settings, width=8, default="normal")
    saveBtn.grid(row=3, column=1, sticky="ew")
    lb2 = Label(save,  text="Load settings:").grid(row=4, column=1, sticky="ew")
    loadBtn = Button(save, text=_("Load"), command=load_settings, width=8, default="normal")
...
def main():
    if len(sys.argv) > 1:
        ...
        from tkinter import filedialog
        from PIL import Image
        im_name = filedialog.askopenfilename(defaultextension=".png",
            filetypes = (
